{*********************************************************}
{    Turbo Pascal-Routinen zum Identifizieren von 189     }
{       verschiedenen Typen komprimierter Dateien         }
{                                                         }
{    Turbo Pascal routines to identify 189 different      }
{            types of compressed files                    }
{                                                         }
{           Version 3.15.01 / 10.10.2003                  }
{                             10-10-2003                  }
{                                                         }
...
{                                werden identifiziert                     }
{          3.15.00   12.08.03  - UHBC 1.0-Format von Uwe Herklotz wird    }
{                                identifiziert                            }
{          3.15.01   08.10.03  - BZip2-Methode von PKZip 6 wird erkannt   }
...
{                                are recognized                           }
{          3.15.00   08-12-03  - UHBC 1.0 format by Uwe Herklotz is       }
{                                identified                               }
{          3.15.01   10-08-03  - BZip2 method of PKZip 6 is recognized    }

Unit IDPacker;

{X+,A+}
{$IFNDEF DEBUG}
{$I comp.pas}           { multiplatform, tp, fpc, vpc }
{$D-,L-}
{$ENDIF}
...
CONST nIDPACKER=189+2;  { changed, rar - 0808-2001 }
...
TYPE PNamArray=Array[1..255] of String[20];
...
PROCEDURE AssignPackerNames;
 BEGIN
  ...
  PackerNames^[188] := 'UHBC';
  PackerNames^[189] := 'PKZip6/BZip2';
  {$IFNDEF ENGLISH}
...
Begin
 ...
 {$ENDIF}
  ...
  If CheckAnySign(152859,'**ACE',5) Then goto THE_END;
         ...
         Begin
           ...
           If (p=35) Or (q In [24..39]) Or (r In [24..31,82..89]) Or (o=1) Then
             Begin
               ...
               Exit;
             End;
           If (Pos('PK'#3#4,IDStr)=1) and (Pos('BZh',IDStr)<>0) Then
            Begin
             ArchiveType := PKBZType;
             Exit;
            End;
